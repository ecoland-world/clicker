...
export default class Business extends Phaser.GameObjects.Container {
  constructor(params) {
    ...
    this.add(this.icon);
    this.icon.updateInvestmentLabel(
      gs.bs[this.key].investments,
      unlockValues[this.nextUnlockIndex],
      unlockValues[this.nextUnlockIndex - 1] || 0
    );
    ...
    let revenue = this.updateTimers
      ? this.baseRevenue * gs.bs[this.key].investments * 0.01
    ...
    this.overlayLocked = params.scene.add
      .sprite(3, 0, "businnesDisabled")
      .setOrigin(0);
  ...
  updateCost() {
    this.cost = Math.round(
      Math.pow(this.coefficient, gs.bs[this.key].investments) * this.baseCost
    );
  ...
  checkUnlock() {
    if (gs.bs[this.key].investments != unlockValues[this.nextUnlockIndex])
      return;
  ...
  checkCosts() {
    const investEnabled = this.investButton.setEnabled(
      gs.stats.game.money >= this.cost
    );
    if (investEnabled) {
    ...
    if (!gs.bs[this.key].manager) {
      const managerEnabled = this.managerContainer.setEnabled(
        gs.stats.game.money >= this.managerCost
      );
      if (managerEnabled) {
  ...
  evaluateUpdateTimers() {
    this.updateTimers =
      !gs.bs[this.key].manager || this.baseTime / gs.bs[this.key].speed > 900;
  }
