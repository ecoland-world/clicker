...
export default class GameScene extends MainScene {
  constructor() {
    super({ key: "gameScene" });
    this.businesses = {};
  }
  ...
  create(params) {
    ...
    // coin emitter
    this.coinsEmitter = this.add.particles(0, 0, "coin", {
      x: 120,
      y: 300,
      lifespan: 2000,
      gravityY: 1000,
      ...
      blendMode: "ADD", // If you want to use blend mode
    });

    // Optionally, start the emitter with no particles initially
  ...
  setupBusinesses() {
    ...
    if (idleEarnings) {
      ...
      gs.stats.notification.text = `We made ${gold.value}${
  ...
  addMoneySpent(money) {
    ...
    if (
      gs.stats.game.moneySpent * 0.01 >
      Math.pow(1.8, gs.stats.character.level + 8)
    ) {
      this.levelUp();
    }
  }
  ...
  checkInvesment(business) {
    ...
    this.coinsEmitter.explode(
      constants.BUSINESSES[business.key].coinsEmit,
      business.x + 90,
      business.y + 60
    );
  }
